...
Now that we have the handle, lets start getting the headers, sections and symbols.

```cpp
Elf64_Ehdr* elf = reinterpret_cast<Elf64_Ehdr*>(handle);
Elf64_Phdr* program = reinterpret_cast<Elf64_Phdr*>(handle + elf->e_phoff);
Elf64_Shdr* section = reinterpret_cast<Elf64_Shdr*>(handle + elf->e_shoff);
```
